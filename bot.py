...
async def render_final_settings_panel(message, data, owner_id):
    ...
    is_public = (scope == 'global')

    scope_text = "🌐 إذاعة عامة" if is_public else "📍 مسابقة داخلية"
    type_text  = "عام 🌐" if is_public else "خاص 📍"
...
# --- 7️⃣ استقبال الاسم والحفظ النهائي مع القالب الجديد ---
...
async def process_quiz_name_final(message: types.Message, state: FSMContext):
    ...
    if not quiz_name:
        return await message.answer("⚠️ أرسل اسم صحيح للمسابقة.")
    ...
    # 3️⃣ النطاق الموحد
    scope = data.get('quiz_scope', 'local')  # local / global
    if scope not in ['local', 'global']:
        scope = 'local'

    is_public = (scope == 'global')

    # 4️⃣ تجهيز القيم الرقمية بأمان
    try:
    ...
    # 5️⃣ بناء الـ Payload المتوافق 100% مع الجدول
    payload = {
        "created_by": u_id,
        "quiz_name": quiz_name,
        "chat_id": u_id,
    ...
    try:
        ...
        await message.answer(
            ...
            f"🏷 الاسم: {quiz_name}\n"
            f"⏱ الوقت: {time_limit} ثانية\n"
            f"📊 عدد الأسئلة: {questions_count}\n"
            f"{scope_text}\n\n"
            f"🚀 اكتب كلمة مسابقة لعرضها."
        )

        await state.finish()

    except Exception as e:
        await message.answer(f"❌ خطأ في الحفظ:\n{str(e)}")
# ==========================================
