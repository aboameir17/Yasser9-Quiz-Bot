...
# ==========================================
# 🧠 العقل المدبر للإذاعة العامة (نسخة ياسر المطور)
# ==========================================
global_quiz = {
    "active": False,        # حالة الرادار (شغال أو طافي)
    "ans": "",             # الإجابة الصحيحة (المسجلة من ميزان العدل)
    "participants": [],    # قائمة ID المجموعات اللي استلمت السؤال
    "winner_id": None,     # ID البطل اللي جاوب أول واحد
    "winner_name": "",     # اسم البطل (لعرضه في لوحة المبدعين)
    "winner_group": "",    # اسم القروب اللي فاز منه (للحماس بين القروبات)
    "start_time": 0,       # وقت انطلاق السؤال (لحساب سرعة البديهة)
    "hint_sent": False,    # هل تم إرسال التلميح الذكي لهذا السؤال؟
    "mode": "السرعة ⚡"     # وضع المسابقة الافتراضي للإذاعة
}

# مصفوفة تراكمية لحفظ نقاط الإذاعة (ترتيب الكوكب)
...
async def start_broadcast_process(c: types.CallbackQuery, quiz_id, owner_id):
    """محرك الإذاعة العامة - إرسال الدعوات بتنسيق ياسر المطور 📊"""
    ...
    # 3. تجهيز نص الإعلان (نفس تنسيقك)
    source_text = "أسئلة البوت الذكية 🤖" if q.get('is_bot_quiz') else "أسئلة المستخدم الخاصة 👤"
    broadcast_text = (
        ...
        f"🔢 عدد الأسئلة: `{q.get('questions_count', 10)}`\n"
        f"⏳ وقت السؤال: `{q.get('time_limit', 15)} ثانية`\n"
        f"━━━━━━━━━━━━━━\n"
        f"⏳ **الانطلاق خلال: 60 ثانية**"
    )
    ...
    for g in groups_res.data:
        try:
            msg = await bot.send_message(g['group_id'], broadcast_text, reply_markup=kb, parse_mode="Markdown")
            invitation_messages.append((g['group_id'], msg.message_id))
        except: continue
    
    await c.answer(f"🚀 تم بث الدعوة لـ {len(invitation_messages)} مجموعة!", show_alert=True)

    # 4. انتظار تجميع المشاركين (بدون تجميد البوت بالكامل)
    await asyncio.sleep(60)

    # 5. 🔥 [الإصلاح]: استدعاء العد التنازلي يدوياً وتمرير البيانات له
    # قبل المسح من قاعدة البيانات، نتأكد من جلب المشاركين
    participants_res = supabase.table("quiz_participants").select("chat_id").eq("quiz_id", quiz_id).execute()
    
    if not participants_res.data:
        # إذا لم يقبل أحد التحدي، نحذف رسائل الدعوة ونعتذر
        for chat_id, msg_id in invitation_messages:
            try: await bot.delete_message(chat_id, msg_id)
            except: pass
        return await bot.send_message(owner_id, "⚠️ تم إلغاء الإذاعة لعدم وجود مشاركين.")

    # تجهيز قائمة الـ IDs العالمية
    global_quiz["participants"] = [p['chat_id'] for p in participants_res.data]

    # حذف رسائل الإعلان فوراً
    for chat_id, msg_id in invitation_messages:
        try: asyncio.create_task(bot.delete_message(chat_id, msg_id))
        except: pass
...
async def engine_broadcast_global(quiz_data, owner_name):
    """
    هذا المحرك يقوم بـ:
    1. اختيار الأسئلة عشوائياً من قاعدة البيانات (مرة واحدة فقط).
    2. تمرير هذه الأسئلة لمحرك الإذاعة العالمي ليرسلها لكل القروبات.
    """
    try:
        # --- [ المرحلة الأولى: الاختيار العشوائي ] ---
        ...
        # تنظيف وجلب الأقسام (منطق ياسر المطور)
        raw_cats = quiz_data.get('cats', [])
        if isinstance(raw_cats, str):
        ...
        cat_ids = [int(c) for c in cat_ids_list if str(c).strip().isdigit()]
        if not cat_ids:
            logging.error("⚠️ خطأ الإذاعة: لم يتم العثور على أقسام.")
            return

        # سحب كل الأسئلة المتاحة في هذه الأقسام
        if is_bot:
        ...
        if not res.data:
            logging.error("⚠️ خطأ الإذاعة: قاعدة البيانات فارغة.")
            return

        # 🔥 هنا "العشوائية": نخلط الكل ونختار العدد المطلوب فقط
        questions_pool = res.data
        random.shuffle(questions_pool)
        count = int(quiz_data.get('questions_count', 10))
        selected_questions = questions_pool[:count]

        # --- [ المرحلة الثانية: إطلاق البث الموحد ] ---
        # الآن نرسل القائمة الجاهزة والعشوائية لمحرك الإذاعة العالمي
        engine_type = "bot" if is_bot else "user"
        
        # استدعاء المحرك الذي يرسل نفس السؤال لكل القروبات في نفس اللحظة
...
async def delete_after(message, delay):
    ...
    try: 
        ...
        pass
# ==========================================
# 🛰️ محرك الإذاعة المطور (النسخة النهائية)
# ==========================================
async def run_global_broadcast_logic(questions, quiz_data, owner_name, engine_type):
    ...
    for i, q in enumerate(questions):
        ...
        # 3. بث السؤال لكل المجموعات 🚀
        send_tasks = [send_quiz_question(cid, q, i+1, len(questions), {
            'owner_name': owner_name, 'mode': quiz_data['mode'], 
            'time_limit': quiz_data['time_limit'], 'cat_name': cat_name
        }) for cid in global_quiz["participants"]]
        ...
        # 6. 🔥 عرض قالبك المطور (لوحة المبدعين + الترتيب)
        res_tasks = [send_creative_results(cid, ans, current_winners, overall_global_scores) 
                     for cid in global_quiz["participants"]]
        await asyncio.gather(*res_tasks, return_exceptions=True)
        
        # ⏱️ [ 7. محرك العد التنازلي بين الأسئلة ] ⏱️
        if i < len(questions) - 1:
            # إرسال رسائل العداد الأولية
            count_tasks = [bot.send_message(cid, "⌛ استعدوا.. السؤال التالي يبدأ بعد 5 ثواني...") 
                           for cid in global_quiz["participants"]]
            ...
            await asyncio.sleep(1)
            # تنظيف رسائل العداد قبل السؤال الجديد
            for m in countdown_msgs:
                if isinstance(m, types.Message): asyncio.create_task(m.delete())
        
    # 8. إعلان لوحة الشرف النهائية
    final_tasks = [send_final_results(cid, overall_global_scores, len(questions)) 
                   for cid in global_quiz["participants"]]
