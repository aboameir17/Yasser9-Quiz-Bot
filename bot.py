...
active_quizzes = {}
# ==========================================
# [0] العقل المركزي للإذاعة العامة (Global Session) 🧠
# ==========================================
# هذا القاموس هو "الرادار" الذي تراقبه كل المجموعات في نفس اللحظة
global_session = {
    "active": False,        # هل يوجد سؤال عالمي شغال الآن؟
    "question": None,       # نص السؤال الحالي
    "answer": None,         # الإجابة الصحيحة (بعد المعالجة بميزان العدل)
    "start_time": None,     # وقت انطلاق السؤال لحساب سرعة البديهة
    "winner": None,         # معلومات الفائز (اسمه، مجموعته، وقته)
    "participants": [],     # قائمة الـ chat_id للمجموعات اللي قبلت التحدي
    "question_time": 15,    # مدة السؤال (ثانية)
    "mode": "السرعة ⚡",      # وضع المسابقة
    "hint_sent": False      # هل تم إرسال تلميح لهذا السؤال؟
}
# ==========================================
...
async def launch_global_countdown(quiz_id, q_data):
    """محرك العد التنازلي الذكي + إرسال سؤال موحد لكل القروبات"""

    # 1️⃣ جلب المجموعات التي ضغطت قبول التحدي
    participants = supabase.table("quiz_participants")\
        .select("chat_id")\
        .eq("quiz_id", quiz_id)\
        .execute()
    ...
    # 3️⃣ العد التنازلي (تعديل نفس الرسالة)
    for icon in timer_icons:
        edit_tasks = []

        text = (
            f"⏳ **المسابقة العالمية تبدأ خلال:** {icon}"
            if icon != "🚀"
            else "🔥 **انطـــلاق! أظهروا لنا قوتكم..**"
        )
    ...
    # 4️⃣ 🔥 اختيار سؤال واحد فقط (حل مشكلة الخلط)
    import random

    all_questions = q_data.get("questions", [])
    if not all_questions:
        logging.error("لا توجد أسئلة في q_data")
        return
    ...
    # 5️⃣ إرسال نفس السؤال لكل القروبات
    send_tasks = []
    ...
    # 6️⃣ تنظيف الجدول المؤقت
    supabase.table("quiz_participants")\
        .delete()\
        .eq("quiz_id", quiz_id)\
        .execute()
# ==========================================
