...
async def send_creative_results(chat_id, correct_ans, winners, group_scores, wrong_answers=None, is_public=False):
    ...
    # 1️⃣ شرط المتفوقين (ترتيب حسب سرعة الإجابة في هذه الجولة)
    if winners:
        msg += "❃┄┄┄┈•{المتفوقين🧑‍🎓}•┈┄┄┄❃\n"
        # الفائزين في هذا السؤال مرتبين حسب الأسرع
    ...
    # 2️⃣ شرط أصحاب الإجابات الخاطئة (في هذا السؤال فقط)
    if wrong_answers:
        msg += "❃┄┄┄┈•{خاطئة❌}•┈┄┄┄❃\n"
        for name in wrong_answers:
            msg += f"👤 {name}\n"
        msg += "❃┅┅┅┄┄┄┈•❃•┈┄┄┄┅┅┅❃\n"

    # 3️⃣ شرط الترتيب (نقاط المسابقة الحالية فقط)
    msg += "\n❃┄┄┄┈•{الترتيب📊}•┈┄┄┄❃\n"
    
    if is_public:
        # --- [ النطاق العام: ترتيب المجموعات ولاعبيها في هذه الجولة ] ---
        for gid, players in group_scores.items():
            if not players: continue
            
            # حساب نقاط المجموعة "الآن" فقط
            current_g_pts = sum(p['points'] for p in players.values())
            
            # جلب اسم المجموعة (ممكن من الذاكرة أو قاعدة البيانات للاسم فقط)
            msg += f"🏘 <b>المجموعة:</b> {gid}\n"
            msg += f"🏆 نقاط الجولة: <b>{current_g_pts}</b>\n"
            
            # ترتيب اللاعبين حسب نقاطهم في المسابقة الحالية
        ...
        # --- [ النطاق الخاص: ترتيب اللاعبين في المسابقة الحالية ] ---
        # نفترض أن group_scores هنا هو قاموس اللاعبين في الشات الحالي
...
async def run_universal_logic(chat_ids, questions, quiz_data, owner_name, engine_type):
    ...
    for i, q in enumerate(questions):
        ...
        if i < total_q - 1:
            ...
            # انتظار هادئ بعد آخر سؤال لإعطاء هيبة للختام
            await asyncio.sleep(2)

    # ======================================================
    # --- [ 🧹 اللمسة الأخيرة: تنظيف الشات (حذف الأسئلة والأجوبة) ] ---
    # ======================================================
    for cid in chat_ids:
        # نقوم بحذف كل الرسائل المخزنة في قائمة messages_to_delete لهذه المجموعة
        for mid in messages_to_delete.get(cid, []):
            try:
                await bot.delete_message(cid, mid)
            except Exception as e:
                # نتخطى الخطأ إذا كانت الرسالة محذوفة مسبقاً
                pass
    ...
    for cid in chat_ids:
        # 1. جلب سكور هذه المجموعة (النقاط المستقلة للجولة الحالية)
        final_scores = group_scores.get(cid, {})
        
        # 2. استدعاء لوحة الشرف النهائية في شات "نظيف" تماماً ✨
        if final_scores:
            # نمرر is_pub ليعرف القالب هل يعرض ترتيب مجموعات أم أفراد
    ...
    # 🚀 3. ترحيل النقاط إلى جدول groups_hub (المهمة الكبرى)
    try:
        await sync_points_to_db(group_scores, is_pub)
    except Exception as e:
        logging.error(f"⚠️ فشل ترحيل النقاط لقاعدة البيانات: {e}")

    # 4. تنظيف الذاكرة المؤقتة (Ready for the next round!)
    for cid in chat_ids:
        if cid in active_quizzes:
