...
async def render_final_settings_panel(message, data, owner_id):
    ...
    scope = data.get('scope', 'local')  # 'global' أو 'local'
    
    q_hint_text = "مفعل ✅" if is_hint else "معطل ❌"
    q_scope_text = "🌐 إذاعة عامة" if scope == 'global' else "📍 مسابقة داخلية"
    ...
    kb.row(InlineKeyboardButton(f"📡 النطاق: {q_scope_text}", callback_data=f"tog_scope_{owner_id}"))
...
# --- [ 5 + 6 ] المحرك الموحد ومعالج الحفظ النهائي مع دعم النطاق --- #
...
async def quiz_settings_engines(c: types.CallbackQuery, state: FSMContext):
    ...
    if action in ['tog', 'cyc', 'set']:
        ...
        # محرك النطاق (scope: global / local)
        elif action == 'tog' and data_parts[1] == 'scope':
            curr_scope = data.get('quiz_scope', 'local')
            new_scope = 'global' if curr_scope == 'local' else 'local'
            await state.update_data(quiz_scope=new_scope)
            await c.answer(f"🌐 تم تغيير النطاق إلى: {'إذاعة عامة 🌍' if new_scope=='global' else 'مسابقة خاصة 📍'}")

        # محرك عدد الأسئلة
        elif action == 'set' and data_parts[1] == 'cnt':
            await state.update_data(quiz_count=int(data_parts[2]))

        # تحديث اللوحة فوراً بعد أي تعديل
...
async def handle_secure_actions(c: types.CallbackQuery, state: FSMContext):
    try:
        ...
        # # 2️⃣ لوحة الإعدادات مع دعم quiz_scope و quiz_type
            ...
            # النطاق والنوع
            is_public = q.get('is_public', False)  # True/False
            quiz_scope = q.get('quiz_scope', 'خاص')  # 'عام' / 'خاص'
            quiz_type = q.get('quiz_type', 'private')  # 'public' / 'private'

            # نصوص للعرض
            scope_text = f"{quiz_scope} 🌐" if quiz_scope == 'عام' else f"{quiz_scope} 📍"
            type_text = "عام 🌐" if quiz_type == 'public' else "خاص 📍"

            text = (
                f"❃┏━━━━━ إعدادات: {q['quiz_name']} ━━━━━┓❃\n"
                f"📊 عدد الأسئلة: {q_count}\n"
                f"📡 النطاق: {scope_text}\n"
                f"🏷 نوع المسابقة: {type_text}\n"
                f"🔖 النظام: {q_mode}\n"
                f"⏳ المهلة: {q_time} ثانية\n"
            ...
            kb.row(
                InlineKeyboardButton(f"📡 النطاق: {scope_text}", callback_data=f"toggle_scope_{quiz_id}_{user_id}"),
                InlineKeyboardButton(f"🏷 النوع: {type_text}", callback_data=f"toggle_type_{quiz_id}_{user_id}")
            )
        ...
        # 3️⃣ التبديلات (Toggles) مع دعم quiz_scope و quiz_type
        elif any(c.data.startswith(x) for x in ['toggle_hint_', 'toggle_speed_', 'toggle_scope_', 'toggle_type_', 'set_c_']):
            quiz_id = data_parts[2]

            # محرك النطاق (Scope) - الآن يدعم is_public و quiz_scope
            if 'toggle_scope_' in c.data:
                res = supabase.table("saved_quizzes").select("is_public", "quiz_scope").eq("id", quiz_id).single().execute()
                curr_is_public = res.data.get('is_public', False)
                curr_scope = res.data.get('quiz_scope', 'خاص')

                # عكس القيم
                new_is_public = not curr_is_public
                new_scope = 'عام' if curr_scope == 'خاص' else 'خاص'

                supabase.table("saved_quizzes").update({"is_public": new_is_public, "quiz_scope": new_scope}).eq("id", quiz_id).execute()
                status_text = f"{new_scope} 🌐" if new_scope == 'عام' else f"{new_scope} 📍"
                await c.answer(f"✅ أصبح النطاق: {status_text}")

            # محرك النوع (quiz_type)
            elif 'toggle_type_' in c.data:
                res = supabase.table("saved_quizzes").select("quiz_type").eq("id", quiz_id).single().execute()
                curr_type = res.data.get('quiz_type', 'private')
                new_type = 'public' if curr_type == 'private' else 'private'
                supabase.table("saved_quizzes").update({"quiz_type": new_type}).eq("id", quiz_id).execute()
                type_text = "عام 🌐" if new_type == 'public' else "خاص 📍"
                await c.answer(f"✅ أصبح النوع: {type_text}")
            ...
            # إعادة توجيه ذاتي لتحديث لوحة الإعدادات فوراً
       ...
       # --- [ نظام تشغيل المسابقات: عامة أو خاصة مع دعم quiz_scope و quiz_type ] ---
        elif c.data.startswith('run_'):
            quiz_id = data_parts[1]
            user_id = data_parts[2]

            # 1️⃣ جلب بيانات المسابقة لمرة واحدة فقط
            res = supabase.table("saved_quizzes").select("*").eq("id", quiz_id).single().execute()
            q_data = res.data
            ...
            # 2️⃣ التحقق من النطاق والنوع
            quiz_scope = q_data.get('quiz_scope', 'خاص')       # 'عام' / 'خاص'
            quiz_type = q_data.get('quiz_type', 'private')     # 'public' / 'private'

            # إذا كانت عامة أو نوعها public
            if q_data.get('is_public') or quiz_type == 'public' or quiz_scope == 'عام':
                await c.answer("🌐 جاري إطلاق الإذاعة العامة للمجموعات...")
                await start_broadcast_process(c, quiz_id, user_id)
            else:
                # تشغيل خاص داخل الشات
